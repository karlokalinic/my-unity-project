...
public class HxVolumetricCamera : MonoBehaviour
{
	...
	{
		if (Mycamera.stereoTargetEye == StereoTargetEyeMask.Both && Application.isPlaying && XRSettings.enabled)
		{
			return true;
		}
		return false;
	...
	{
		if (Mycamera.stereoTargetEye != StereoTargetEyeMask.None && Application.isPlaying && XRSettings.enabled)
		{
	...
	private void CreateTempTextures()
	{
		...
		if (downScaledBlurRT == null && (blurCount > 0 || ((BlurTransparency > 0 || MapToLDR) && TransparencySupport)) && resolution != Resolution.full)
		{
		...
		if (FullBlurRT2 == null && ((resolution != Resolution.full && UpSampledblurCount > 0) || (resolution == Resolution.full && (blurCount > 0 || ((BlurTransparency > 0 || MapToLDR) && TransparencySupport) || TemporalSampling)) || MapToLDR))
		{
	...
	private void CreateFinalizeBuffer()
	{
		...
		if ((BlurTransparency > 0 || MapToLDR) && TransparencySupport)
		{
			...
			for (int i = 0; i < num + 1; i++)
			{
				...
				{
					BufferFinalize.SetRenderTarget((resolution != Resolution.full) ? downScaledBlurRTID : FullBlurRT2ID);
					...
					BufferFinalize.SetGlobalTexture("_MainTex", (resolution != Resolution.full) ? downScaledBlurRTID : FullBlurRT2ID);
	...
	private void BuildBuffer()
	{
		...
		CurrentTint = new Vector3(((QualitySettings.activeColorSpace != ColorSpace.Gamma) ? TintColor.linear : TintColor).r, ((QualitySettings.activeColorSpace != ColorSpace.Gamma) ? TintColor.linear : TintColor).g, ((QualitySettings.activeColorSpace != ColorSpace.Gamma) ? TintColor.linear : TintColor).b) * TintIntensity;
		CurrentTintEdge = new Vector3(((QualitySettings.activeColorSpace != ColorSpace.Gamma) ? TintColor2.linear : TintColor2).r, ((QualitySettings.activeColorSpace != ColorSpace.Gamma) ? TintColor2.linear : TintColor2).g, ((QualitySettings.activeColorSpace != ColorSpace.Gamma) ? TintColor2.linear : TintColor2).b) * TintIntensity;
		if (!dirty)
		{
			return;
		}
		...
		if (Mycamera.stereoTargetEye != StereoTargetEyeMask.None && Application.isPlaying && XRSettings.enabled)
		{
			...
			else
			{
				...
				worldToCameraMatrix3[12] += Mycamera.stereoSeparation / 2f * (float)((stereoscopicEye != Camera.StereoscopicEye.Left) ? 1 : (-1));
