...
public class HxVolumetricLight : MonoBehaviour
{
	...
	private LightType lastType = LightType.Rectangle;
	...
	private Color LightColor()
	{
		if (myLight != null)
		{
			return (QualitySettings.activeColorSpace != ColorSpace.Gamma) ? myLight.color.linear : myLight.color;
		}
		if (myDummyLight != null)
		{
			return (QualitySettings.activeColorSpace != ColorSpace.Gamma) ? myDummyLight.color.linear : myDummyLight.color;
		}
		if (myProjector != null)
		{
			return (QualitySettings.activeColorSpace != ColorSpace.Gamma) ? myProjector.material.GetColor("_Color").linear : myProjector.material.GetColor("_Color");
		}
		return Color.white;
	}
	...
	{
		bool flag = LightShadow() != LightShadows.None && Shadows;
		if (!dirty)
		{
			return;
		}
	...
	private void BuildSpotLightBuffer(CommandBuffer cameraBuffer)
	{
		...
		cameraBuffer.DrawMesh(HxVolumetricCamera.SpotLightMesh, LightMatrix, HxVolumetricCamera.GetSpotMaterial(mid), 0, (!(lastBounds.SqrDistance(HxVolumetricCamera.Active.transform.position) < HxVolumetricCamera.ActiveCamera.nearClipPlane * 2f * (HxVolumetricCamera.ActiveCamera.nearClipPlane * 2f))) ? 1 : 0, propertyBlock);
		if (flag)
		{
	...
	private Vector3 CalcTintColor(Vector4 c)
	{
		...
		if (CustomTintColor)
		{
			vector += new Vector3(((QualitySettings.activeColorSpace != ColorSpace.Gamma) ? TintColor.linear : TintColor).r, ((QualitySettings.activeColorSpace != ColorSpace.Gamma) ? TintColor.linear : TintColor).g, ((QualitySettings.activeColorSpace != ColorSpace.Gamma) ? TintColor.linear : TintColor).b) * ((!CustomTintIntensity) ? HxVolumetricCamera.Active.TintIntensity : TintIntensity);
		}
		else
		{
	...
	private Vector3 CalcTintColorEdge(Vector4 c)
	{
		...
		if (CustomTintColor2)
		{
			vector += new Vector3(((QualitySettings.activeColorSpace != ColorSpace.Gamma) ? TintColor2.linear : TintColor2).r, ((QualitySettings.activeColorSpace != ColorSpace.Gamma) ? TintColor2.linear : TintColor2).g, ((QualitySettings.activeColorSpace != ColorSpace.Gamma) ? TintColor2.linear : TintColor2).b) * ((!CustomTintIntensity) ? HxVolumetricCamera.Active.TintIntensity : TintIntensity);
		}
		else
		{
	...
	private void BuildPointBuffer(CommandBuffer cameraBuffer)
	{
		...
		int shaderPass = ((!(num <= LightRange() + LightRange() * 0.09f + HxVolumetricCamera.ActiveCamera.nearClipPlane * 2f)) ? 1 : 0);
		if (propertyBlock == null)
		{
			propertyBlock = new MaterialPropertyBlock();
		}
		...
		if (LightCookie() != null)
		{
			...
			cameraBuffer.DrawMesh(HxVolumetricCamera.SphereMesh, LightMatrix, HxVolumetricCamera.GetPointMaterial(mid), 0, shaderPass, propertyBlock);
		}
		else
		{
			cameraBuffer.DrawMesh(HxVolumetricCamera.SphereMesh, LightMatrix, HxVolumetricCamera.GetPointMaterial(mid), 0, shaderPass, propertyBlock);
	...
	private void UpdateLightMatrix()
	{
		...
		else if (GetLightType() == LightType.Rectangle && myProjector != null)
		{
			if (myProjector.orthographic)
			{
	...
	{
		if (!first && !base.transform.hasChanged && !matrixReconstruct && LastRange == LightRange() && LastSpotAngle == LightSpotAngle() && lastType == GetLightType() && (GetLightType() != LightType.Rectangle || (LastAspect == GetAspect() && LastOrthoSize != GetOrthoSize() && LastOrtho != GetOrtho())))
		{
			return;
		}
		if (GetLightType() == LightType.Point)
