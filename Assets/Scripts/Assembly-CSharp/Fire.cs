...
public class Fire : LightBehavior
{
	...
	private float UpdateLightSource()
	{
		...
		{
			base.ChangeValue = (0f - Mathf.Sin(base.TimePassed * 8f * (float)Math.PI - 1.5f)) * 0.07f + 0.93f;
		...
		{
			base.ChangeValue = (0f - Mathf.Sin(base.TimePassed * 8f * (float)Math.PI - 1.5f)) * 0.03f + 0.97f;
		}
