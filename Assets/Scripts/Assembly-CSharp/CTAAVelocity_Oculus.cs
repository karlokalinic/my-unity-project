...
public class CTAAVelocity_Oculus : MonoBehaviour
{
	...
	{
		Matrix4x4 stereoProjectionMatrix = GetComponent<Camera>().GetStereoProjectionMatrix(VRCameraEYE);
		...
		CTAAVRJitter_Oculus component = GetComponent<CTAAVRJitter_Oculus>();
		if (component != null)
		{
			velocityMaterial.SetVector("_Corner", GetPerspectiveProjectionCornerRay(_camera, component.getActiveSampleX(), component.getActiveSampleY()));
		}
		else
		{
		...
		FullScreenQuad();
		List<CTAA_Dynamic_Tag> activeObjects = CTAA_Dynamic_Tag.activeObjects;
		int i = 0;
		for (int count = activeObjects.Count; i != count; i++)
		{
			CTAA_Dynamic_Tag cTAA_Dynamic_Tag = activeObjects[i];
			if (cTAA_Dynamic_Tag != null && !cTAA_Dynamic_Tag.sleeping && cTAA_Dynamic_Tag.mesh != null)
			{
				velocityMaterial.SetMatrix("_CurrM", cTAA_Dynamic_Tag.localToWorldCurr);
				velocityMaterial.SetMatrix("_PrevM", cTAA_Dynamic_Tag.localToWorldPrev);
				velocityMaterial.SetPass((!cTAA_Dynamic_Tag.useSkinnedMesh) ? 1 : 2);
				for (int j = 0; j != cTAA_Dynamic_Tag.mesh.subMeshCount; j++)
				{
					Graphics.DrawMeshNow(cTAA_Dynamic_Tag.mesh, Matrix4x4.identity, j);
				}
			}
		}
		RenderTexture.active = active;
		velocityViewMatrix = worldToCameraMatrix;
		GetComponent<Camera>().SetStereoProjectionMatrix(VRCameraEYE, stereoProjectionMatrix);
	}
}
